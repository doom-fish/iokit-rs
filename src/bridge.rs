//! Internal FFI declarations for the crate's Swift bridge.
...
unsafe extern "C" {
    ...
    /// Wraps the Swift bridge's `iokit_swift_service_matching_entry_id` entry point.
    pub fn iokit_swift_service_matching_entry_id(entry_id: u64) -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_matching_services` entry point.
    pub fn iokit_swift_matching_services(name: *const c_char) -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_name_matching_services` entry point.
    pub fn iokit_swift_name_matching_services(name: *const c_char) -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_wrap_service` entry point.
    pub fn iokit_swift_wrap_service(service: u32) -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_service_raw` entry point.
    pub fn iokit_swift_service_raw(service: *mut c_void) -> u32;
    /// Wraps the Swift bridge's `iokit_swift_service_retain` entry point.
    pub fn iokit_swift_service_retain(service: *mut c_void) -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_service_release` entry point.
    pub fn iokit_swift_service_release(service: *mut c_void);
    /// Wraps the Swift bridge's `iokit_swift_service_class_name` entry point.
    pub fn iokit_swift_service_class_name(service: *mut c_void) -> *mut c_char;
    /// Wraps the Swift bridge's `iokit_swift_service_bundle_identifier` entry point.
    pub fn iokit_swift_service_bundle_identifier(service: *mut c_void) -> *mut c_char;
    /// Wraps the Swift bridge's `iokit_swift_service_superclass_name` entry point.
    pub fn iokit_swift_service_superclass_name(service: *mut c_void) -> *mut c_char;
    /// Wraps the Swift bridge's `iokit_swift_service_conforms_to` entry point.
    pub fn iokit_swift_service_conforms_to(service: *mut c_void, class_name: *const c_char)
        -> bool;
    /// Wraps the Swift bridge's `iokit_swift_service_is_equal_to` entry point.
    pub fn iokit_swift_service_is_equal_to(service: *mut c_void, other: *mut c_void) -> bool;
    /// Wraps the Swift bridge's `iokit_swift_service_kernel_retain_count` entry point.
    pub fn iokit_swift_service_kernel_retain_count(service: *mut c_void) -> u32;
    /// Wraps the Swift bridge's `iokit_swift_service_user_retain_count` entry point.
    pub fn iokit_swift_service_user_retain_count(service: *mut c_void) -> u32;
    /// Wraps the Swift bridge's `iokit_swift_service_retain_count` entry point.
    pub fn iokit_swift_service_retain_count(service: *mut c_void) -> u32;
    /// Wraps the Swift bridge's `iokit_swift_service_busy_state` entry point.
    pub fn iokit_swift_service_busy_state(service: *mut c_void, busy_state: *mut u32) -> i32;
    /// Wraps the Swift bridge's `iokit_swift_service_wait_quiet` entry point.
    pub fn iokit_swift_service_wait_quiet(service: *mut c_void, seconds: u32) -> i32;
    /// Wraps the Swift bridge's `iokit_swift_service_authorize` entry point.
    pub fn iokit_swift_service_authorize(service: *mut c_void, options: u32) -> i32;
    /// Wraps the Swift bridge's `iokit_swift_service_open` entry point.
    pub fn iokit_swift_service_open(
        service: *mut c_void,
        ty: u32,
        connect_out: *mut *mut c_void,
    ) -> i32;
    /// Wraps the Swift bridge's `iokit_swift_service_as_registry_entry` entry point.
    pub fn iokit_swift_service_as_registry_entry(service: *mut c_void) -> *mut c_void;

    /// Wraps the Swift bridge's `iokit_swift_wrap_registry_entry` entry point.
    pub fn iokit_swift_wrap_registry_entry(entry: u32) -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_registry_entry_from_path` entry point.
    pub fn iokit_swift_registry_entry_from_path(path: *const c_char) -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_registry_entry_retain` entry point.
    pub fn iokit_swift_registry_entry_retain(entry: *mut c_void) -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_registry_entry_release` entry point.
    pub fn iokit_swift_registry_entry_release(entry: *mut c_void);
    /// Wraps the Swift bridge's `iokit_swift_registry_entry_name` entry point.
    pub fn iokit_swift_registry_entry_name(entry: *mut c_void) -> *mut c_char;
    /// Wraps the Swift bridge's `iokit_swift_registry_entry_name_in_plane` entry point.
    pub fn iokit_swift_registry_entry_name_in_plane(
        entry: *mut c_void,
        plane: *const c_char,
    ) -> *mut c_char;
    /// Wraps the Swift bridge's `iokit_swift_registry_entry_location_in_plane` entry point.
    pub fn iokit_swift_registry_entry_location_in_plane(
        entry: *mut c_void,
        plane: *const c_char,
    ) -> *mut c_char;
    /// Wraps the Swift bridge's `iokit_swift_registry_entry_path` entry point.
    pub fn iokit_swift_registry_entry_path(entry: *mut c_void, plane: *const c_char)
        -> *mut c_char;
    /// Wraps the Swift bridge's `iokit_swift_registry_entry_registry_entry_id` entry point.
    pub fn iokit_swift_registry_entry_registry_entry_id(entry: *mut c_void, out: *mut u64) -> i32;
    /// Wraps the Swift bridge's `iokit_swift_registry_entry_properties` entry point.
    pub fn iokit_swift_registry_entry_properties(entry: *mut c_void) -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_registry_entry_property` entry point.
    pub fn iokit_swift_registry_entry_property(
        entry: *mut c_void,
        key: *const c_char,
    ) -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_registry_entry_search_property` entry point.
    ...
    /// Wraps the Swift bridge's `iokit_swift_registry_entry_parent` entry point.
    pub fn iokit_swift_registry_entry_parent(
        entry: *mut c_void,
        plane: *const c_char,
    ) -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_registry_entry_child` entry point.
    pub fn iokit_swift_registry_entry_child(
        entry: *mut c_void,
        plane: *const c_char,
    ) -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_registry_entry_parent_iterator` entry point.
    pub fn iokit_swift_registry_entry_parent_iterator(
        entry: *mut c_void,
        plane: *const c_char,
    ) -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_registry_entry_child_iterator` entry point.
    pub fn iokit_swift_registry_entry_child_iterator(
        entry: *mut c_void,
        plane: *const c_char,
    ) -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_registry_entry_create_iterator` entry point.
    pub fn iokit_swift_registry_entry_create_iterator(
        entry: *mut c_void,
        plane: *const c_char,
        options: u32,
    ) -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_registry_entry_in_plane` entry point.
    ...
    /// Wraps the Swift bridge's `iokit_swift_iterator_enter_entry` entry point.
    pub fn iokit_swift_iterator_enter_entry(iterator: *mut c_void) -> i32;
    /// Wraps the Swift bridge's `iokit_swift_iterator_exit_entry` entry point.
    pub fn iokit_swift_iterator_exit_entry(iterator: *mut c_void) -> i32;
    /// Wraps the Swift bridge's `iokit_swift_iterator_reset` entry point.
    pub fn iokit_swift_iterator_reset(iterator: *mut c_void);
    /// Wraps the Swift bridge's `iokit_swift_iterator_next_service` entry point.
    pub fn iokit_swift_iterator_next_service(iterator: *mut c_void) -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_iterator_next_registry_entry` entry point.
    pub fn iokit_swift_iterator_next_registry_entry(iterator: *mut c_void) -> *mut c_void;

    /// Wraps the Swift bridge's `iokit_swift_notification_port_create` entry point.
    pub fn iokit_swift_notification_port_create() -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_notification_port_retain` entry point.
    pub fn iokit_swift_notification_port_retain(port: *mut c_void) -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_notification_port_release` entry point.
    pub fn iokit_swift_notification_port_release(port: *mut c_void);
    /// Wraps the Swift bridge's `iokit_swift_notification_port_mach_port` entry point.
    pub fn iokit_swift_notification_port_mach_port(port: *mut c_void) -> u32;
    /// Wraps the Swift bridge's `iokit_swift_notification_port_run_loop_source` entry point.
    ...
    /// Wraps the Swift bridge's `iokit_swift_connect_release` entry point.
    pub fn iokit_swift_connect_release(connect: *mut c_void);
    /// Wraps the Swift bridge's `iokit_swift_connect_get_service` entry point.
    pub fn iokit_swift_connect_get_service(connect: *mut c_void) -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_connect_set_notification_port` entry point.
    ...
    /// Wraps the Swift bridge's `iokit_swift_connect_add_client` entry point.
    pub fn iokit_swift_connect_add_client(connect: *mut c_void, client: *mut c_void) -> i32;
    /// Wraps the Swift bridge's `iokit_swift_connect_call_scalar_method` entry point.
    ...
    /// Wraps the Swift bridge's `iokit_swift_connect_call_struct_method` entry point.
    ...
    /// Wraps the Swift bridge's `iokit_swift_connect_call_method` entry point.
    ...
    /// Wraps the Swift bridge's `iokit_swift_power_find_power_management` entry point.
    pub fn iokit_swift_power_find_power_management() -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_power_sleep_enabled` entry point.
    pub fn iokit_swift_power_sleep_enabled() -> bool;
    /// Wraps the Swift bridge's `iokit_swift_power_get_aggressiveness` entry point.
    pub fn iokit_swift_power_get_aggressiveness(
        connect: *mut c_void,
        ty: u64,
        value_out: *mut u64,
    ) -> i32;
    /// Wraps the Swift bridge's `iokit_swift_power_set_aggressiveness` entry point.
    pub fn iokit_swift_power_set_aggressiveness(connect: *mut c_void, ty: u64, value: u64) -> i32;
    /// Wraps the Swift bridge's `iokit_swift_power_get_thermal_warning_level` entry point.
    pub fn iokit_swift_power_get_thermal_warning_level(level_out: *mut u32) -> i32;
    /// Wraps the Swift bridge's `iokit_swift_power_copy_assertions_by_process` entry point.
    pub fn iokit_swift_power_copy_assertions_by_process() -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_power_copy_assertions_status` entry point.
    pub fn iokit_swift_power_copy_assertions_status() -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_power_copy_battery_info` entry point.
    pub fn iokit_swift_power_copy_battery_info() -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_power_copy_cpu_power_status` entry point.
    pub fn iokit_swift_power_copy_cpu_power_status() -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_power_copy_scheduled_power_events` entry point.
    pub fn iokit_swift_power_copy_scheduled_power_events() -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_power_assertion_create` entry point.
    pub fn iokit_swift_power_assertion_create(
        assertion_type: *const c_char,
        level: u32,
        name: *const c_char,
    ) -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_power_assertion_declare_user_activity` entry point.
    pub fn iokit_swift_power_assertion_declare_user_activity(
        name: *const c_char,
        user_type: u32,
    ) -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_power_assertion_declare_network_client_activity` entry point.
    pub fn iokit_swift_power_assertion_declare_network_client_activity(
        name: *const c_char,
    ) -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_power_assertion_retain` entry point.
    pub fn iokit_swift_power_assertion_retain(assertion: *mut c_void) -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_power_assertion_release` entry point.
    pub fn iokit_swift_power_assertion_release(assertion: *mut c_void);
    /// Wraps the Swift bridge's `iokit_swift_power_assertion_id` entry point.
    pub fn iokit_swift_power_assertion_id(assertion: *mut c_void) -> u32;
    /// Wraps the Swift bridge's `iokit_swift_power_assertion_copy_properties` entry point.
    pub fn iokit_swift_power_assertion_copy_properties(assertion: *mut c_void) -> *mut c_void;

    /// Wraps the Swift bridge's `iokit_swift_iops_power_sources_info_create` entry point.
    pub fn iokit_swift_iops_power_sources_info_create() -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_iops_power_sources_info_retain` entry point.
    pub fn iokit_swift_iops_power_sources_info_retain(info: *mut c_void) -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_iops_power_sources_info_release` entry point.
    pub fn iokit_swift_iops_power_sources_info_release(info: *mut c_void);
    /// Wraps the Swift bridge's `iokit_swift_iops_power_sources_info_count` entry point.
    pub fn iokit_swift_iops_power_sources_info_count(info: *mut c_void) -> usize;
    /// Wraps the Swift bridge's `iokit_swift_iops_power_sources_info_description` entry point.
    pub fn iokit_swift_iops_power_sources_info_description(
        info: *mut c_void,
        index: usize,
    ) -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_iops_power_sources_info_provider_type` entry point.
    pub fn iokit_swift_iops_power_sources_info_provider_type(info: *mut c_void) -> *mut c_char;
    /// Wraps the Swift bridge's `iokit_swift_iops_copy_external_power_adapter_details` entry point.
    pub fn iokit_swift_iops_copy_external_power_adapter_details() -> *mut c_void;
    /// Wraps the Swift bridge's `iokit_swift_iops_get_time_remaining_estimate` entry point.
    pub fn iokit_swift_iops_get_time_remaining_estimate() -> f64;
    /// Wraps the Swift bridge's `iokit_swift_iops_get_battery_warning_level` entry point.
    pub fn iokit_swift_iops_get_battery_warning_level() -> u32;

    /// Wraps the Swift bridge's `iokit_swift_io_message_constant_count` entry point.
    pub fn iokit_swift_io_message_constant_count() -> u32;
    /// Wraps the Swift bridge's `iokit_swift_io_message_constant` entry point.
    pub fn iokit_swift_io_message_constant(index: u32) -> u32;

    /// Wraps the Swift bridge's `iokit_swift_iohi_backing_store_public_sdk_available` entry point.
    pub fn iokit_swift_iohi_backing_store_public_sdk_available() -> bool;
    /// Wraps the Swift bridge's `iokit_swift_iohi_backing_store_unavailability_reason` entry point.
